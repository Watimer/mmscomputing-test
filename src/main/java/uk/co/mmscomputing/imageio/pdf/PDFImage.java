...
public class PDFImage extends PDFXObject{
  ...
  public PDFImage(String name, BufferedImage image) throws IOException{    // Writer
    ...
    put("Name",             new PDFName(name));
    put("Width",            new PDFInteger(width));
    put("Height",           new PDFInteger(height));

    ByteArrayOutputStream baos = new ByteArrayOutputStream();
    ColorModel            cm   = image.getColorModel();
    if((image.getType()==BufferedImage.TYPE_BYTE_BINARY)&&(cm.getPixelSize()==1)){

      put("BitsPerComponent", new PDFInteger(1));
      put("ColorSpace",       new PDFName("DeviceGray"));
...
//      ccittDict.put("K",new PDFObject.PDFInteger(0));                    // ModHuffmanOutputStream
      ccittDict.put("K",new PDFInteger(-1));                     // ModModREADOutputStream
      ccittDict.put("Columns",new PDFInteger(width));
      ccittDict.put("Rows",new PDFInteger(height));
      ...
      if(image.getType()==BufferedImage.TYPE_BYTE_GRAY){                   // one component; grey scale
        put("BitsPerComponent", new PDFInteger(8));
        put("ColorSpace",       new PDFName("DeviceGray"));

        String[] filters = {/*"ASCII85Decode",*/"DCTDecode"};              // /Filter /ASCII85Decode /DCTDecode
        setFilters(filters);
      }else{                                                               // three components; YCbCr
        put("BitsPerComponent", new PDFInteger(8));              // only RGB for the moment
...
//    filter
    ...
    if(filter.equals("DCTDecode")){
      String colorspace = ((PDFName)get("ColorSpace")).getName();
...
//        int bpc    = ((PDFObject.PDFInteger)get("BitsPerComponent")).getValue();
  ...
  public BufferedImage getImage()throws IOException{
    ...
    int width  = ((PDFInteger)get("Width")).getValue();
    int height = ((PDFInteger)get("Height")).getValue();
